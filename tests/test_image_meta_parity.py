...
# Upstream #308 caps a decompressed zTXt/iTXt value at 1 MiB: PNG text is
# metadata, not a document, and a few hundred KB of crafted deflate expands to
# hundreds of megabytes that the marker scan then copies again.
PNG_TEXT_CAP = 1 << 20
OVER_CAP = zlib.compress(b"Generated by AI " + b"A" * PNG_TEXT_CAP, 9)
AT_CAP = zlib.compress(b"Generated by AI " + b"A" * (PNG_TEXT_CAP - 16), 9)


def half(stream: bytes) -> bytes:
    """Half a deflate stream: a text chunk that stops in the middle.

    #308 also swapped zlib.decompress for a decompressobj, which hands back
    what it managed to decode instead of raising, so markers in the surviving
    prefix are now found where the whole chunk used to be discarded.
    """
    return stream[: len(stream) // 2]


CUT_FLAT = half(zlib.compress(b"Generated by OpenAI Sora " * 100, 9))
CUT_GENERATOR = half(zlib.compress(b"ChatGPT made this image for you " * 100, 9))
...
SAMPLES = {
    ...
    # Upstream #308: over the cap the chunk is refused, not scanned.
    "png_ztxt_over_cap": make_png([(b"zTXt", b"Comment\x00\x00" + OVER_CAP)]),
    "png_itxt_over_cap": make_png([(b"iTXt", b"Comment\x00\x01\x00\x00\x00" + OVER_CAP)]),
    # Sixteen bytes under the cap, so the boundary is asserted, not assumed.
    "png_ztxt_at_cap": make_png([(b"zTXt", b"Comment\x00\x00" + AT_CAP)]),
    # A deflate stream that stops early still yields the prefix it decoded.
    "png_ztxt_cut_stream": make_png([(b"zTXt", b"Comment\x00\x00" + CUT_FLAT)]),
    "png_ztxt_cut_generator": make_png([(b"zTXt", b"Software\x00\x00" + CUT_GENERATOR)]),
    "png_itxt_cut_generator": make_png([(b"iTXt", b"Software\x00\x01\x00\x00\x00" + CUT_GENERATOR)]),
...
def test_png_text_over_the_cap_is_refused_rather_than_inflated() -> None:
    """Upstream #308, stated as behaviour rather than as parity.

    A zTXt/iTXt value that inflates past 1 MiB is never allocated, so nothing
    about it can be ruled out: inspect says the chunk was not fully inspected
    instead of leaving it out of the findings, and a keep-mode clean drops it
    rather than keeping a chunk it could not read. One byte under the cap, the
    ordinary path still runs and still finds the marker inside.
    """
    for name in ("png_ztxt_over_cap", "png_itxt_over_cap"):
        ctype = "zTXt" if "ztxt" in name else "iTXt"
        src = SAMPLES[name]
        js = _js("inspect", src, {})
        assert js["findings"] == [f"PNG {ctype}: not fully inspected (decompressed text exceeds cap)"], name
        assert js["has_ai_metadata"] is False, name
        kept = _js("clean", src, {"stripAllMetadata": False})
        assert kept["actions"] == [f"drop chunk {ctype} (decompressed text exceeds cap)"], name
        assert OVER_CAP not in base64.b64decode(kept["data"]), name

    at_cap = _js("inspect", SAMPLES["png_ztxt_at_cap"], {})
    assert at_cap["findings"] == ["PNG zTXt: Generated by"]
    assert at_cap["has_ai_metadata"] is True


def test_a_text_chunk_that_stops_early_is_scanned_as_far_as_it_decoded() -> None:
    """The markers are only in the decompressed prefix, not in the raw chunk."""
    assert _js("inspect", SAMPLES["png_ztxt_cut_stream"], {})["findings"] == [
        "PNG zTXt: Generated by, OpenAI"]
    for name, ctype in (("png_ztxt_cut_generator", "zTXt"), ("png_itxt_cut_generator", "iTXt")):
        assert _js("inspect", SAMPLES[name], {})["findings"] == [
            f"PNG {ctype}: AI generator (Software=ChatGPT)"], name
