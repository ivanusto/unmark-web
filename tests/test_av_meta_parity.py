...
def truncated_id3v2(frames: bytes, *, major: int = 4, declared: int = 10_000) -> bytes:
    """A tag header claiming far more tag than the bytes that follow it.

    Upstream #201: the frame boundaries of such a tag are unknowable, so it is
    reported as truncated and dropped up to the first audio frame, rather than
    silently yielding nothing.
    """
    return b"ID3" + bytes([major, 0, 0]) + syncsafe(declared) + frames


def id3v2_frame(frame_id: bytes, payload: bytes, *, major: int = 4) -> bytes:
    size = syncsafe(len(payload)) if major == 4 else struct.pack(">I", len(payload))
...
SAMPLES: dict[str, bytes] = {
    ...
    # The chunk is intact; the ID3v2 tag inside it is not. _inspect_id3v2 is
    # shared, so the truncation report reaches the WAV findings too.
    "wav_id3_truncated": make_wav([(b"id3 ", truncated_id3v2(id3v2_frame(b"TSSE", AI_TEXT)))]),
    ...
    # ---- MP3 with a truncated ID3v2 tag (upstream #201)
    "mp3_truncated_ai": truncated_id3v2(id3v2_frame(b"TSSE", AI_TEXT)) + MPEG_FRAME * 2,
    "mp3_truncated_benign": truncated_id3v2(id3v2_frame(b"TIT2", b"\x00A song")) + MPEG_FRAME * 2,
    "mp3_truncated_v23": truncated_id3v2(id3v2_frame(b"TSSE", AI_TEXT, major=3), major=3) + MPEG_FRAME,
    ...
    "mp3_truncated_no_audio": truncated_id3v2(id3v2_frame(b"TSSE", AI_TEXT)),
    # A sync word whose bitrate index is the "bad" 1111: not a frame header,
    # and the scan has to keep looking past it.
    "mp3_truncated_fake_sync": (truncated_id3v2(id3v2_frame(b"TSSE", AI_TEXT))
                                + b"\xff\xfb\xf0\x00" + MPEG_FRAME),
    # ---- FLAC
...
def test_truncated_id3v2_tag_keeps_the_audio_and_says_so() -> None:
    """Upstream #201, stated as behaviour rather than as parity.

    A tag whose declared size overruns the file cannot be decomposed into
    frames, so the old path found nothing and reported the file clean. It is
    now reported as truncated, and the clean drops the unreadable tag up to the
    first valid MPEG frame header instead of trusting a length it cannot check.
    A sync word with a reserved bitrate index is not that header.
    """
    for name, offset in (("mp3_truncated_ai", 45), ("mp3_truncated_fake_sync", 49)):
        src = SAMPLES[name]
        for mode in ("inspect", "inspect-file"):
            findings = _js(mode, src)["findings"]
            assert findings[0].startswith("truncated ID3v2.4 tag detected"), (name, mode)
        for strip_all in (True, False):
            for mode in ("clean", "clean-file"):
                js = _js(mode, src, {"stripAllMetadata": strip_all})
                out = base64.b64decode(js["data"])
                assert js["actions"] == [
                    f"drop truncated ID3v2.4 tag (found audio frame at offset {offset})"], (name, mode)
                assert out == src[offset:], (name, mode, strip_all)
                assert out.startswith(MPEG_FRAME[:4]), (name, mode, strip_all)


def test_a_truncated_tag_with_no_audio_to_find_is_left_alone() -> None:
    """Cleaning must not be able to empty a file it cannot parse."""
    src = SAMPLES["mp3_truncated_no_audio"]
    for mode in ("clean", "clean-file"):
        js = _js(mode, src, {"stripAllMetadata": True})
        assert js["actions"] == [
            "cannot locate valid audio frame in truncated ID3v2.4 tag; preserving file"], mode
        assert base64.b64decode(js["data"]) == src, mode
