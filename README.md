...
- `js/engine.js`, `js/engine_ops.js`, `js/worker.js`: the engine work runs in a Web Worker when the browser allows one. `engine_ops.js` holds the op table with no DOM in it and is loaded by both sides, so the worker path and the same-thread path run the same code rather than two copies of it. A page opened straight off the filesystem cannot start a worker: `engine.js` then calls the same table in place, and callers cannot tell the difference.
