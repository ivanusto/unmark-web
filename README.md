...
- **The Inspector no longer reads a file on the thread that draws the page.** It read every image whole, then had the bytes copied again into the worker, and neither copy was of any use to the page: only the engine wants them. The `File` goes across instead and is read where it is used. A 24 MiB PNG now inspects in about 335 ms rather than 390 ms, with one allocation of it instead of two.
